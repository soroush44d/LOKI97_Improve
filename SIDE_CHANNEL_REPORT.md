...
## نکات سازگاری که اصلاح شدند
- نوع `ULONG64` از `unsigned long` به `uint32_t` برای هر نیمه تغییر داده شد تا پیاده‌سازی در معماری‌های ۶۴بیتی هم رفتار مورد انتظار (دو نیمه ۳۲بیتی) داشته باشد.
- بعد از این اصلاح، تست `self_test` (در `test_ecb`) دوباره پاس شد و round-trip رمز/بازگشایی درست کار می‌کند.
